...
if (isset($_GET['mid']) && isset($_GET['workfield'])) {
    ...
    if ($workField == 0) {
        ...
        $query = "SELECT  cpu.mid  as `MID`,c.brandname  as `Brand`,cpu.subbrand  as `Sub Brand`,cpu.brandmodifier  as `Brand Modifier`,cpu.skunumber as `SKU Number`,cpu.numberofcores  as `Number of Cores`,cpu.numberofthreads  as `Number of Threads`,cpu.clockspeed  as `Clock Speed`,cpu.cache  as `Cache`,c.body,c.title,c.price,c.quantity,c.imageurls,c.discount FROM cpu , product as c WHERE c.mid = '" . $mid . "' and cpu.mid = c.mid";
    } else if ($workField == 2) {
        $query = "SELECT gpu.mid  as `MID`,g.brandname  as `Brand`,gpu.subbrand  as `Sub Brand`,gpu.brandmodifier  as `Brand Modifier`,gpu.skunumber as `SKU Number`,gpu.generation  as `Generation`,gpu.vram  as `Vram`,g.body,g.title,g.price,g.quantity,g.imageurls,g.discount FROM gpu , product as g WHERE g.mid = '" . $mid . "' and gpu.mid = g.mid";
    } else if ($workField == 3) {
        $query = "SELECT ram.mid  as `MID`,r.brandname  as `Brand`,ram.generation  as `Generation`,ram.writespeed  as `Write Speed`,ram.readspeed  as `Read Speed`,ram.capacity  as `Capacity`,r.body,r.title,r.price,r.quantity,r.imageurls,r.discount FROM ram , product as r WHERE r.mid = '" . $mid . "' and ram.mid = r.mid";
    } else if ($workField == 4) {
        $query = "SELECT storage.mid  as `MID`,s.brandname  as `Brand`,storage.type  as `Type`,storage.writespeed  as `Write Speed`,storage.readspeed  as `Read Speed`,storage.capacity  as `Capacity`,storage.color  as `Color`,s.body,s.title,s.price,s.quantity,s.imageurls,s.discount FROM storage , product as s WHERE s.mid = '" . $mid . "' and storage.mid = s.mid";
    }
